...
const ProductDetails = () => {
    ...
    const { addToCart, mainRequest, chosenDelivery, handleDeliveryChange, websiteInfo, ad } = useContext(AppContext);
    const [userQuantity, setUserQuantity] = useState(1);
    const [product, setProduct] = useState({});
    const [loading, setLoading] = useState(false);

    const { images, title, price, quantity, start_date, total_price, fake_visitor, visitor, category, category_id, stars = 5, small_description, discount, add_image, add_link } = product;
                    ...
                    <div className='container py-10'>
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-10 relative">
                            {images && images.length > 0 && (
                                <div className="img-holder lg:sticky top-0 h-fit p-5 col-span-3 md:col-span-2 lg:col-span-1 flex justify-center rounded-lg max-h-[400px]">
                                    <Swiper
                                        slidesPerView={1}
                                        spaceBetween={30}
                                        loop={true}
                            ...
                            <div className="product-details col-span-3 md:col-span-2 lg:col-span-1 bg-[#e5e5e5] grid grid-cols-1 gap-5 p-5 rounded-lg">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    {discount && (
                                        <span className='text-md text-red-500 ms-auto line-through'>{`${i18n.language === "ar" ? `جنية ${discount}` : `${discount} EGY`} `}</span>
                                    )
                                    }
                                </div>
                                {
                            ...
                            <div className="col-span-3 lg:col-span-1 h-fit">
                                ...
                                </div>
                                {
                                    add_image ? (
                                        <a href={add_link}>
                                            <img className='p-5 min-h-[400px] h-[400px] mx-auto' src={`https://admin.horriizon.com/public/${add_image}`} alt="Horizon" />
                                        </a>
                                    )
                                        :
                                        (
