...
const SingleProductCard = ({ background, id, title, total_price, price, quantity, image, category, allProducts }) => {
    ...
    return (
        <div loading="lazy" className={`product-card flex flex-col items-start justify-between min-h-[460px] relative bg-${background} ${background ? "py-3 px-2" : ""}`}>
            ...
            <div className={`flex flex-col gap-2 w-full p-2  justify-end ${i18n.language === "ar" ? "text-end" : "text-start"}`}>
                ...
                <div className="details h-full flex flex-col items-start justify-end gap-5 flex-1 w-full mt-1">
                    ...
                    <button disabled={loading} onClick={() => { addToCart(id) }} aria-label='add to cart' className="cart-icon hover:bg-mainColor duration-200 flex items-center justify-center gap-2 bg-secondColor text-white text-sm p-2 rounded-md w-full">
